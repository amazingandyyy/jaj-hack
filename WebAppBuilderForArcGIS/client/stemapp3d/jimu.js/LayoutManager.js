...
function(declare, lang, array, html, _WidgetBase, topic, on, query, domConstruct, domGeometry,
  ...
  clazz = declare([_WidgetBase], {
    constructor: function(options, domId) {
      ...
      this.own(topic.subscribe("sceneViewLoaded", lang.hitch(this, this.onSceneViewLoaded)));
      this.own(topic.subscribe("sceneViewChanged", lang.hitch(this, this.onSceneViewChanged)));
      ...
      this.own(topic.subscribe("beforeSceneViewDestory",
                               lang.hitch(this, this.onBeforeSceneViewDestroy)));
    ...
    onSceneViewLoaded: function(sceneView){
      this.sceneView = sceneView;
      this.panelManager.setSceneView(this.sceneView);
      this._loadPreloadWidgets(this.appConfig);
    },

    onSceneViewChanged: function(sceneView){
      this.sceneView = sceneView;
      this.panelManager.setSceneView(this.sceneView);
      this._loadPreloadWidgets(this.appConfig);
    },

    onBeforeSceneViewDestroy: function(){
    ...
    _onLayoutChange: function(appConfig){
      topic.publish("beforeLayoutChange");
      ...
      topic.publish("afterLayoutChange");
    },
    ...
    _getFixedThemeStyles: function(theme){
      ...
      if(theme.name === 'PlateauTheme'){
        ...
        cssText += '.jimu-widget-homebutton .HomeButton .home, .jimu-widget-mylocation,' +
          ' .jimu-widget-mylocation .place-holder, .jimu-widget-zoomslider.vertical .zoom-in,' +
          ' .jimu-widget-zoomslider.vertical .zoom-out, .jimu-widget-home, .jimu-widget-navigate,' +
          ' .jimu-widget-zoom .esri-zoom .esri-widget, .jimu-widget-mylocation, ' +
          '.jimu-widget-search .esri-search-button.esri-submit,.jimu-widget-placeholder,.jimu-widget-onscreen-icon' +
          '{background-color: ${mainBackgroundColor} !important;}';
        cssText += '.jimu-widget-onscreen-icon' +
          '{background-color: ${mainBackgroundColor} !important;}';
      }else if(theme.name === 'BoxTheme'){
        cssText += '.jimu-main-background, .jimu-widget-home, .jimu-widget-navigate, ' +
        '.jimu-widget-zoom .esri-zoom .esri-widget, .jimu-widget-mylocation, ' +
        '.jimu-widget-search .esri-search-button.esri-submit, .jimu-widget-placeholder' +
        '{background-color: ${mainBackgroundColor} !important;}';
    ...
    _createPreloadWidgetPlaceHolder: function(widgetConfig){
      ...
      cfg.position.width = 32;
      cfg.position.height = 32;
    ...
    _createPreloadWidgetIcon: function(widgetConfig){
      ...
      // }));
      html.setStyle(iconDijit.domNode, {
        width: "32px",
        height: "32px"
      });
